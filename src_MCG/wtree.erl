...
-export [alive/1, broadcast/3, children/1, create/1, create/0, init/1,
	 ...
	 update/4, update/3, retrieve/4, retrieve/3, retrieve/2,
...
try_it(Fun, Args, Who, What) ->
  Error = fun(Class, Reason) ->
    failure("error in " ++ Who ++ " function for wtree:" ++ What, Class, Reason)
  end,
...
failure(ErrMsg, Class, Reason) ->
  {fail, [self(), ErrMsg, Class, Reason, workers:get_stacktrace()]}.
...
%% TODO:  We should use the tree structure when sending the messages.
%%   THe current version (calling workers:update) has a race with tree-operations
%%   such as reduce and scan.  I've never seen a race-error "in the wild" from this,
%%   but, fixig it would be a good idea.
update(W, Key, Fun, Args) -> workers:update(W, Key, Fun, Args).

%% @spec  update(W, Key, X) -> ok
%% @equiv workers:update(W, Key, X)
%% TODO:  Make a native tree-style implementation.  See the notes above.
update(W, Key, X) -> workers:update(W, Key, X).

%% @spec retrieve(W, Fun, Args, TimeOut) -> Values::List
%% @equiv workers:retrieve(W, Fun, Args, TimeOut)
%% TODO:  Make a native tree-style implementation.
retrieve(W, Fun, Args, TimeOut) -> workers:retrieve(W, Fun, Args, TimeOut).

%% @spec retrieve(W, Fun, X) -> Values::List
%% @equiv workers:retrieve(W, Fun, X)
%% TODO:  Make a native tree-style implementation.
retrieve(W, Fun, X) -> workers:retrieve(W, Fun, X).

%% @spec retrieve(W, X) -> Values::List
%% @equiv workers:retrieve(W, X)
%% TODO:  Make a native tree-style implementation.
retrieve(W, X) -> workers:retrieve(W, X).
